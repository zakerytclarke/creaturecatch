...
export class MenuScene extends Phaser.Scene {
  ...
  create(data: { tab?: Tab }) {
    ...
    this.add.rectangle(0, 0, GAME_WIDTH, GAME_HEIGHT, 0x0a0f1a, 0.94).setOrigin(0);
    panel(this, 6, 6, GAME_WIDTH - 12, GAME_HEIGHT - 12, { fill: 0x1a2238, radius: 10 });
    ...
    tabs.forEach((t, i) => {
      new Button(this, 12 + i * 68, 12, t.label, {
        ...
        h: 24,
        size: 11,
        fill: t.id === this.tab ? 0x5081d6 : 0x39415c,
  ...
  private renderCreatureList(list: CreatureInstance[], kind: 'party' | 'box') {
    ...
    list.forEach((c, i) => {
      ...
      panel(this, 12, y, GAME_WIDTH - 24, 38, { fill: 0x222c46, radius: 6, stroke: TYPE_META[sp.types[0]].color });
      this.add.image(34, y + 19, `creature_${c.speciesId}`).setScale(0.5);
      label(this, 60, y + 5, `${displayName(c)}  Lv${c.level}`, { size: 12, bold: true });
  ...
  private showDetail(c: CreatureInstance) {
    ...
    overlay.add(panel(this, 40, 30, GAME_WIDTH - 80, GAME_HEIGHT - 60, { radius: 10 }));
    const sp = speciesOf(c);
    overlay.add(this.add.image(90, 90, `creature_${c.speciesId}`).setScale(1.4));
    overlay.add(label(this, 150, 46, `${displayName(c)}`, { size: 16, bold: true }));
    ...
    overlay.add(
      label(
        ...
        { size: 11, color: '#dfe6f5' },
      ),
    );
    overlay.add(label(this, 56, 140, 'Moves:', { size: 12, bold: true }));
  ...
  private renderBag() {
    ...
    owned.forEach((b, i) => {
      ...
      panel(this, 12, y, GAME_WIDTH - 24, 30, { fill: 0x222c46, radius: 6 });
      label(this, 20, y + 4, `${item.name}  x${b.qty}`, { size: 12, bold: true });
      label(this, 20, y + 18, item.description, { size: 9, color: '#9fb0d0' });
    });
  }
  ...
  private renderShop() {
    ...
    SHOP_ITEMS.forEach((id, i) => {
      ...
      panel(this, 12, y, GAME_WIDTH - 24, 30, { fill: 0x222c46, radius: 6 });
      label(this, 20, y + 4, `${item.name}`, { size: 12, bold: true });
      label(this, 20, y + 18, `¢${item.price}  •  ${item.description}`, { size: 9, color: '#9fb0d0' });
